...
const POST_TOOLS: ToolDef[] = [
  {
    ...
    function: {
      name: "findProduct",
      description:
        "Research the product on the client website. Uses user-provided images first. Returns needsDescription when copy would require guessing; noUsableImage when info exists but no photo.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "writeCaption",
      description: "Write the Facebook caption for this post using product and business context.",
  ...
  },
  {
    ...
    function: {
      name: "askUser",
      description:
        "Pause and ask the user a clarifying question when product research fails or no usable product image is available. Offer choices when appropriate (upload a photo OR reply generate).",
      parameters: {
        type: "object",
...
3. After findProduct:
   - If needsDescription is true: call askUser with suggestedQuestion (e.g. "Tell me briefly what X is…"). Do NOT call writeCaption or makeGraphic until the user explains the product.
   ...
   - If found:true with a reliable description: proceed to writeCaption.
4. After askUser resumes, read the tool result:
   ...
   - If choice is "upload" or imageUrls present: user attached photo(s) — if description is included, call writeCaption then makeGraphic; if needsDescription, ask again for what the product is.
...
5. writeCaption() — only after a marketing brief exists (from site research, user reply, or web enrichment). Photo vision alone is not enough — ask the user if blocked.
6. makeGraphic() — generate the graphic last. If the user chose "generate" without uploading, proceed prompt-only (no product reference required).

You MUST finish every post with writeCaption then makeGraphic, OR call askUser if blocked. NEVER end with a plain-text reply — if you cannot proceed, call askUser instead of stopping.
...
async function buildPostSystemPrompt(projectId: string, taskId: string): Promise<string> {
  const refs = await getReferencesForTask(projectId, taskId);
  const refSummary = formatProjectReferencesSummary(refs);
  if (!refSummary) return SYSTEM_PROMPT;
  return `${SYSTEM_PROMPT}\n\n${refSummary}`;
}

const STATUS_LABELS: Record<string, string> = {
  findProduct: "Creating post — researching product…",
  writeCaption: "Creating post — writing caption…",
...
async function taskHasDeliverable(
  taskId: string
  ...
  const graphic = task.generations[0];
  if (!graphic?.imagePath) {
...
async function tryAutoFinishDeliverable(
  ...
  if (!task.caption?.trim() && autoContinueDepth < 1 && state.messages.length > 0) {
    ...
    const systemPrompt = await buildPostSystemPrompt(task.projectId, taskId);
...
async function finalizePostAgentRun(
  ...
  if (!deliverable.ok) {
    await updateTaskFields(taskId, {
      status: "FAILED",
      statusLabel: deliverable.reason,
...
export async function continuePostAgentAfterImageRequest(
  ...
  try {
    const parsed = JSON.parse(userReply) as { choice?: string; pendingContextImageId?: string };
    skipImageRequest = parsed.choice === "generate";
...
export async function resumePostAgent(taskId: string, userReply: string): Promise<{
  ...
  const systemPrompt = await buildPostSystemPrompt(taskFresh.projectId, taskId);
  const result = await runAgentLoop(
    createPostLoopOptions({
      taskId,
      projectId: taskFresh.projectId,
